...
const PanditDashboard = ({ pandit, onLogout }) => {
  ...
  const [isOnline, setIsOnline] = useState(false); // Start with false, then sync
  ...
  useEffect(() => {
    const syncOnlineStatus = async () => {
      try {
        ...
        if (data.success) {
          const backendStatus = data.isOnline;
          const cachedStatus = localStorage.getItem('panditOnlineStatus');
          const cachedValue = cachedStatus ? JSON.parse(cachedStatus) : null;
          
          console.log(`🔄 Status sync - Backend: ${backendStatus}, Cached: ${cachedValue}`);
          
          // Use backend status as source of truth
          setIsOnline(backendStatus);
          localStorage.setItem('panditOnlineStatus', JSON.stringify(backendStatus));
          setLastSyncTime(new Date());
          
          // If they differ, log warning
          if (cachedValue !== null && cachedValue !== backendStatus) {
            console.warn(`⚠️ Status mismatch detected! Backend: ${backendStatus}, LocalStorage: ${cachedValue}. Using backend value.`);
          }
        }
      } catch (error) {
        console.error('Failed to sync online status:', error);
        // Fallback to cached or prop value
        const savedStatus = localStorage.getItem('panditOnlineStatus');
        const fallbackStatus = savedStatus ? JSON.parse(savedStatus) : (pandit?.isOnline || false);
    ...
    const syncInterval = setInterval(syncOnlineStatus, 30000);
    
    return () => clearInterval(syncInterval);
  }, [pandit?.isOnline]);
  
  // ✅ Add online status indicator in header
  ...
  useEffect(() => {
    const checkStatusConsistency = async () => {
      ...
      const cachedValue = JSON.parse(cachedStatus);
      if (cachedValue !== isOnline) {
        setSyncWarning(true);
        // Auto-correct after 5 seconds of mismatch
        const timeout = setTimeout(() => {
          if (isOnline !== cachedValue) {
            console.log('🔄 Auto-correcting status mismatch');
            localStorage.setItem('panditOnlineStatus', JSON.stringify(isOnline));
  ...
  const loadBookings = async () => {
    try {
      ...
      // 'pending' tab shows both 'notified' (notified but not accepted) and 'pending' (specific pandit bookings)
      let params = {};
      if (activeTab === 'pending') {
        params = { status: 'notified,pending' };
  ...
  const toastId = toast.loading(`Updating status to ${newStatus ? 'Online' : 'Offline'}...`);
  
  try {
    console.log('🔄 Toggling availability to:', newStatus);
    await panditApi.updateAvailability({ isOnline: newStatus });
    
    // ✅ Verify backend status after update
    const verifyResponse = await fetch(buildUrl('/pandit/online-status'), {
      headers: { 'Authorization': `Bearer ${authStorage.getAuth('pandit').token}` }
    });
    const verifyData = await verifyResponse.json();
    
    if (verifyData.success && verifyData.isOnline !== newStatus) {
      console.warn('⚠️ Backend returned different status, reverting');
      setIsOnline(verifyData.isOnline);
      localStorage.setItem('panditOnlineStatus', JSON.stringify(verifyData.isOnline));
      toast.update(toastId, {
        render: `Status reverted to ${verifyData.isOnline ? 'Online' : 'Offline'} due to server mismatch`,
        type: 'warning',
        isLoading: false,
        autoClose: 3000
      });
      ...
      toast.update(toastId, {
        render: `You are now ${newStatus ? 'Online' : 'Offline'}`,
        type: 'success',
        isLoading: false,
        autoClose: 2000
      });
    ...
    localStorage.setItem('panditOnlineStatus', JSON.stringify(previousStatus));
...
🕉️ Thank you for choosing Pujanam!`;
  ...
  <div className="online-status-container">
    <div className={`status-indicator ${isOnline ? 'online' : 'offline'}`}>
      <div className={`status-dot ${isOnline ? 'online' : 'offline'}`}></div>
      <span>{isOnline ? 'Online - Receiving Bookings' : 'Offline'}</span>
      {isSyncing && <span className="syncing-spinner">⟳</span>}
      {syncWarning && (
        <span className="sync-warning" title="Status mismatch detected">
          ⚠️
        </span>
      )}
    </div>
      ...
      onClick={async () => {
        ...
        try {
          ...
          if (data.success) {
            setIsOnline(data.isOnline);
            localStorage.setItem('panditOnlineStatus', JSON.stringify(data.isOnline));
            toast.success('Status synced with server');
          }
        } catch (error) {
    ...
    >
      <FontAwesomeIcon icon={faSyncAlt} spin={isSyncing} />
    </button>
