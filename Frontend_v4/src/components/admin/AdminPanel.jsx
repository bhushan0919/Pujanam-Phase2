...
import { faArrowRight, faMagnifyingGlass, faScrewdriverWrench, faBook, faTimes, faUserCheck, faHeadset, faCalendar, faNewspaper, faUserPlus, faCircleXmark, faPlus, faHeadSideCough, faPager, faParagraph, faLocation, faLocationDot,faComment, faEnvelope, faPhone, faStar, faGraduationCap, faPencil, faTrash, faCheck, faRupee, faRupeeSign, faIndianRupeeSign, faClock, faFile, faEdit } from '@fortawesome/free-solid-svg-icons';
...
const AdminPanel = () => {
  ...
  }, [activeTab, filters, contactFilter]);
  ...
  const validatePrice = (priceString) => {
    ...
    const numeric = parseInt(match[0].replace(/,/g, ''));
    return numeric > 0;
  };
  ...
  const handleServiceSubmit = async (e) => {
    ...
    try {
      ...
      if (!validatePrice(serviceForm.price)) {
        setMessage('❌ Please enter a valid price (e.g., ₹1,099/- or 1099)');
        return;
      }
  ...
  const editService = (service) => {
    ...
    setImagePreview(null); // existing URL image is shown via serviceForm.image directly
  ...
  const addLanguage = () => {
    if (newLanguage) {
      setPanditForm(prev => {
        if (prev.languages.includes(newLanguage)) return prev;
  ...
  const addDetail = () => {
    if (newDetail) {
      setServiceForm(prev => {
        if (prev.details.includes(newDetail)) return prev;
  ...
  const removeArrayItem = (field, index, setForm) => {
    setForm(prev => ({
      ...prev,
  ...
  const loadSupportTickets = async () => {
    try {
      ...
      const response = await fetch(buildUrl(`/admin/support-tickets${statusParam}`), {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const updateTicketStatus = async (ticketId, status, adminResponse = '') => {
    try {
      ...
      const response = await fetch(buildUrl(`/admin/support-tickets/${ticketId}/status`), {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const adminCancelBooking = async (bookingId, reason) => {
    ...
    try {
      ...
      const response = await fetch(buildUrl(`/admin/bookings/${bookingId}/admin-cancel`), {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const loadContactMessages = async () => {
    try {
      ...
      const response = await fetch(buildUrl(`/contact/admin/messages${statusParam}`), {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const updateMessageStatus = async (messageId, status, adminNoteText = null) => {
    try {
      const { token } = authStorage.getAuth('admin');

      const body = { status };
      if (adminNoteText !== null) body.adminNote = adminNoteText;

      const response = await fetch(buildUrl(`/contact/admin/messages/${messageId}/status`), {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const sendReply = async () => {
    ...
    try {
      ...
      const response = await fetch(buildUrl(`/contact/admin/messages/${selectedMessage._id}/reply`), {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const deleteMessage = async (messageId) => {
    ...
    try {
      ...
      const response = await fetch(buildUrl(` /contact/admin/messages/${messageId}`), {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  // Mark as spam
  const markAsSpam = async (messageId) => {
    if (window.confirm('Mark this message as spam? It will be moved to spam folder.')) {
      await updateMessageStatus(messageId, 'spam');
    }
  };
  ...
  return (
    <div className="admin-panel">
      ...
      <div className="admin-tabs">
        ...
        >
          <FontAwesomeIcon icon={faEnvelope} /> Contact Messages ({contactStats.unread || 0})
        </button>
      </div>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  setPanditForm(prev => ({
                    ...
                    // only auto-generate if username empty
                    username:
                      !prev.username ||
                        prev.username === generateUsername(prev.name)
            ...
            <div className="form-row">
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) => setPanditForm(prev => ({ ...prev, location: e.target.value }))}
                  required
                />
              </div>
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) => setPanditForm(prev => ({ ...prev, email: e.target.value }))}
                  required
                />
              </div>
            </div>
            ...
            <div className="form-row">
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) => setPanditForm(prev => ({ ...prev, username: e.target.value }))}
                  required
                />
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) => setPanditForm(prev => ({ ...prev, password: e.target.value }))}
            ...
            <div className="form-row">
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) => setPanditForm(prev => ({ ...prev, contact: e.target.value }))}
                  required
                />
              </div>
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (file) {
                      ...
                      setPanditForm(prev => ({ ...prev, image: file }));
                    }
                  }}
                />
              ...
              <div className="services-dropdown-container">
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (selectedService) {
                      setPanditForm(prev => {
                        if (prev.services.includes(selectedService)) return prev;
                ...
                {panditForm.services.length === 0 ? (
                  ...
                  panditForm.services.map((service, index) => (
                    <span key={index} className="array-item">
                      ...
                      <button
                        type="button"
                        onClick={() => removeArrayItem('services', index, setPanditForm)}
          ...
          <form onSubmit={handleServiceSubmit} className="admin-form">
            ...
            <div className="form-group">
              ...
              <textarea
                ...
                onChange={(e) => setServiceForm(prev => ({ ...prev, purpose: e.target.value }))}
                rows="2"
              />
            </div>
              ...
              <div className="array-items">
                {serviceForm.details.map((detail, index) => (
                  <span key={index} className="array-item">
                    ...
                    <button
                      type="button"
                      onClick={() => removeArrayItem('details', index, setServiceForm)}
            ...
            <div className="form-group">
              ...
              {/* Inline image preview — shown right after file is selected */}
              {(imagePreview || (serviceForm.image && !(serviceForm.image instanceof File))) && (
                <div className="image-preview">
                  <p>Image Preview:</p>
                  <img
                    src={imagePreview || serviceForm.image}
                    alt="Service preview"
                    style={{ width: '150px', height: '150px', objectFit: 'cover', borderRadius: '8px', marginTop: '8px' }}
                  />
  ...
  <div className="admin-contact-section">
    <div className="section-header">
      <h2><FontAwesomeIcon icon={faEnvelope} /> Contact Messages</h2>
      <div className="contact-filters">
        ...
        <button
          className={contactFilter === 'replied' ? 'active' : ''}
          onClick={() => setContactFilter('replied')}
        >
          Replied ({contactStats.replied || 0})
        </button>
        <button
          className={contactFilter === 'spam' ? 'active' : ''}
          onClick={() => setContactFilter('spam')}
        >
          Spam ({contactStats.spam || 0})
        </button>
      </div>
    </div>
    ...
    {loadingContact ? (
      ...
      <div className="contact-messages-list">
        {contactMessages.map((message) => (
          <div key={message._id} className={`contact-message-card ${message.status}`}>
            ...
            <div className="message-details">
              ...
              <p><strong>IP:</strong> <code className="ip-address">{message.ip}</code></p>
              ...
              {message.adminNote && (
                <div className="admin-note">
                  ...
                  <p>{message.adminNote}</p>
                </div>
              )}
              {message.repliedAt && (
                <div className="replied-info">
                  <strong>Replied on:</strong> {new Date(message.repliedAt).toLocaleString()}
                </div>
              )}
            </div>
            ...
            <div className="message-actions">
              {message.status === 'unread' && (
                <button onClick={() => markAsRead(message._id)} className="btn-mark-read">
                  <FontAwesomeIcon icon={faCheck} /> Mark as Read
                </button>
              )}
              <button onClick={() => openReplyModal(message)} className="btn-reply">
                <FontAwesomeIcon icon={faComment} /> Reply
              </button>
              {message.status !== 'spam' && (
                <button onClick={() => markAsSpam(message._id)} className="btn-spam">
                  <FontAwesomeIcon icon={faTimes} /> Mark as Spam
                </button>
              )}
              <button onClick={() => deleteMessage(message._id)} className="btn-delete">
                <FontAwesomeIcon icon={faTrash} /> Delete
              </button>
            </div>
          </div>
      ...
      <div className="modal-overlay" onClick={() => setReplyModalOpen(false)}>
        <div className="modal-content reply-modal" onClick={e => e.stopPropagation()}>
          <div className="modal-header">
            <h3><FontAwesomeIcon icon={faComment} /> Reply to {selectedMessage.name}</h3>
            <button className="close-btn" onClick={() => setReplyModalOpen(false)}>✕</button>
          </div>
          ...
          <div className="modal-body">
            ...
            <div className="form-group">
              <label>Admin Note (Internal, not sent to user)</label>
              <textarea
                rows="2"
                value={adminNote}
          ...
          <div className="modal-actions">
            <button onClick={() => setReplyModalOpen(false)} className="btn-cancel">
              Cancel
            </button>
            <button onClick={sendReply} disabled={replying} className="btn-send">
