...
import { faCalendar, faUserCircle, faUser, faArrowRight, faCheck, faCancel, faBook, faClock, faLocation, faRupeeSign, faNoteSticky, faInr, faLock, faBell, faPhone, faStar, faGear, faList, faBug, faArrowRightArrowLeft, faComment } from "@fortawesome/free-solid-svg-icons";
...
const UserDashboard = () => {
  ...
  useEffect(() => {
    ...
    // ✅ FIX: Update every minute (60000 ms)
  ...
  const loadBookings = async () => {
    try {
      ...
      if (result.success) {
        ...
        setError(result.message || 'Unable to load dashboard data. Please try again later.');
      }
    } catch (error) {
  ...
  const retryPayment = async (booking) => {
    try {
      ...
      // Get payment order
      const response = await fetch(`${API_CONFIG.BASE_URL}/payment/create-order`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
        ...
        body: JSON.stringify({
          bookingId: booking._id,
          totalAmount: booking.actualPrice || extractPrice(booking.price)
        })
      });
      ...
      script.onload = () => {
        const options = {
          key: orderData.keyId,
          amount: orderData.amount * 100, // Backend returns rupees; Razorpay SDK expects paise
          ...
          handler: async (response) => {
            try {
              const verifyRes = await fetch(`${API_CONFIG.BASE_URL}/payment/verify-payment`, {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                  'Authorization': `Bearer ${token}`
          ...
          modal: {
            ondismiss: async () => {
              setProcessingPayment(null);
              // Cancel the booking when user dismisses the payment modal
              try {
                await fetch(`${API_CONFIG.BASE_URL}/payment/payment-cancelled`, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
                  body: JSON.stringify({ bookingId: booking._id })
                });
                loadBookings(); // Refresh to show cancelled status
        ...
        razorpay.on('payment.failed', async () => {
          alert('❌ Payment failed. Booking has been cancelled. Please create a new booking to try again.');
          setProcessingPayment(null);
  ...
  // Helper function to check if booking is payable
  const isBookingPayable = (booking) => {
    // Must have actualPrice > 0
    if (!booking.actualPrice || booking.actualPrice <= 0) return false;

    // Must have pending payment
    ...
    <div className="user-dashboard">
      ...
      <div className="stats-cards">
        ...
        <div className="stat-card warning">
          <h3>{bookings.filter(b => b.paymentStatus === 'pending' && b.status !== 'cancelled').length}</h3>
          <p>Pending Payment</p>
          <small>Complete payment to confirm</small>
        </div>
                  ...
                  <div className="payment-warning">
                    <div className="warning-banner">
                      ...
                      <div>
                        ...
                        <p>Your booking is not confirmed until payment is completed.</p>
                      </div>
                    </div>
                  ...
                  {isBookingPayable(booking) && (
                    ...
                    >
                      {processingPayment === booking._id ? (
                        ...
                        <>
                          💳 Pay Now
                          <small className="amount-small"> (₹{booking.advanceAmount || Math.round(booking.actualPrice * 0.3)})</small>
                        </>
                      )}
                    </button>
                  )}

                  {booking.paymentStatus === 'pending' && booking.actualPrice === 0 && (
                    <div className="warning-banner error">
                      <span>⚠️</span>
                      <div>
                        <strong>Payment Unavailable</strong>
                        <p>This booking has an invalid price. Please contact support to complete your booking.</p>
                      </div>
                    </div>
                  )}
