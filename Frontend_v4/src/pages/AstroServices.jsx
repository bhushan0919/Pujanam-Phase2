...
const AstroServices = () => {
    ...
    const handleAction = (service) => {
        // Handle Premium Services - Show Coming Soon Modal
        ...
        // Handle Free Services - Navigate with back navigation support
        if (service.tier === 'free') {
            if (!isAuthenticated && service.requiresAuth) {
                localStorage.setItem('redirectAfterLogin', service.actionRoute);
                navigate('/user/login', { 
                    state: { 
                        message: 'Please login to access Astro Services.',
        ...
        if (!isAuthenticated && service.requiresAuth) {
            localStorage.setItem('redirectAfterLogin', service.actionRoute);
            navigate('/user/login', { state: { message: 'Please login to access Astro Services.' } });
            return;
        }
        navigate(service.actionRoute);
    };
    ...
    return (
        <>
            {/* Coming Soon Modal for Premium Services */}
            {showComingSoonModal && selectedPremiumService && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowComingSoonModal(false)}>
                    <div className="mx-4 w-full max-w-md animate-in zoom-in-95 fade-in duration-200" onClick={(e) => e.stopPropagation()}>
                        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-50 to-orange-50 p-6 shadow-2xl">
                            <button
                            ...
                            <div className="text-center">
                                <div className="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br from-amber-200 to-orange-200">
                                    <span className="text-4xl">🚀</span>
                                </div>
                                
                                <h3 className="mb-2 text-2xl font-bold text-amber-800">
                                    Coming Soon!
                                </h3>
                                
                                <p className="mb-4 text-gray-600">
                                    <strong>{selectedPremiumService.title}</strong> is currently in development.
                                </p>
                                
                                <div className="mb-6 rounded-xl bg-amber-100/50 p-4">
                                    <p className="text-sm text-amber-800">
                                        📅 Expected launch: <strong>Coming weeks</strong>
                                    </p>
                                ...
                                <button
                                    onClick={() => setShowComingSoonModal(false)}
                                    className="w-full rounded-full bg-gradient-to-r from-amber-600 to-orange-600 px-6 py-3 font-semibold text-white transition hover:-translate-y-0.5 hover:shadow-lg"
                                >
                                    Got it, thanks! 🙏
                                </button>
                            </div>
            ...
            {toastMessage && (
                <div className="fixed bottom-6 left-1/2 z-50 -translate-x-1/2 animate-in fade-in slide-in-from-bottom-5 duration-300">
                    <div className="flex items-center gap-3 rounded-2xl bg-gradient-to-r from-amber-600 to-orange-600 px-6 py-4 shadow-2xl">
                        <span className="text-2xl">🔮</span>
                        <div>
                            <p className="font-bold text-white">{toastMessage.title}</p>
                            <p className="text-sm text-amber-100">{toastMessage.message}</p>
                        </div>
                        <button 
            ...
            <div className={`min-h-screen overflow-hidden bg-[#fbf0e0] text-[#5b2614] ${isVisible ? 'opacity-100' : 'opacity-0'} transition-opacity duration-500`}>
                ...
                <main className="relative mx-auto max-w-[1240px] px-4 pb-16 pt-8 sm:px-6 lg:px-8">
                    {/* Add padding top to account for fixed back button */}
                        ...
                        <div className="grid items-center gap-10 lg:grid-cols-[1.2fr_0.95fr] lg:gap-8">
                            <div className="max-w-2xl">
                                ...
                                <div className="mt-6 flex flex-wrap items-center gap-x-8 gap-y-3 text-sm text-[#8b6a52]">
                                    ...
                                    <div className="flex items-center gap-2"><span className="text-[#e09d3e]">🪔</span><span>200+ Kundlis Studied</span></div>
                                </div>
                            </div>
