...
exports.updatePandit = async (req, res) => {
  try {
    ...
    if (updateData.password === '********' || !updateData.password) {
      ...
      // findByIdAndUpdate bypasses the pre-save hook, so we hash manually
...
exports.updateBookingStatus = async (req, res) => {
  try {
    ...
    const validStatuses = ['pending', 'notified', 'accepted', 'confirmed', 'completed', 'cancelled'];
...
// NOTE: This function is kept for legacy reference but paymentController uses
// bookingController.afterSuccessfulPayment instead. io is passed via global app instance.
exports.afterSuccessfulPayment = async (bookingId, io = null) => {
  try {
    const booking = await Booking.findById(bookingId);
    if (!booking) return;

    // If no specific pandit was selected, find and notify suitable pandits
    if (!booking.panditId) {
      const NotificationService = require('../utils/notificationService');
      const suitablePandits = await NotificationService.findSuitablePandits(booking);
      
      if (suitablePandits.length > 0) {
        await NotificationService.notifyPandits(booking, suitablePandits, io);
        booking.status = 'notified';
        booking.notifiedPandits = suitablePandits.map(p => p._id);
        await booking.save();
      }
    } else {
      // Notify the specific pandit
    ...
    console.log(`✅ Post-payment processing completed for booking ${bookingId}`);
  } catch (error) {
