...
const _send = async ({ to, toName, subject, html }) => {
  const { BrevoClient } = require('@getbrevo/brevo');
  const client = new BrevoClient({ apiKey: process.env.BREVO_API_KEY });
  return client.transactionalEmails.sendTransacEmail({
    sender: {
      name: process.env.EMAIL_FROM_NAME || 'Pujanam',
...
const sendPanditWelcomeEmail = async (pandit, plainPassword) => {
  try {
    ...
    const mailOptions = {
      from: `"Pujanam Admin" <${process.env.EMAIL_USER}>`,
      to: pandit.email,
      subject: 'Welcome to Pujanam - Your Pandit Account has been Created',
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              ...
              <p>We are pleased to inform you that your details are verified and your Pandit account has been successfully created on the <strong>Pujanam</strong> platform.</p>
              ...
              <div class="spiritual-message">
                ...
                We're honored to have you as part of the Pujanam family."
              </div>
    ...
    await _send({ to: pandit.email, toName: pandit.name, subject: mailOptions.subject, html: mailOptions.html });
...
const sendPanditPasswordResetEmail = async (pandit, resetToken) => {
  try {
    ...
    const mailOptions = {
      from: `"Pujanam Support" <${process.env.EMAIL_USER}>`,
      to: pandit.email,
