...
class NotificationService {
  ...
  try {
    console.log(`📨 Sending notifications to ${pandits.length} pandits`);
      ...
      for (const pandit of pandits) {
        ...
        // ✅ REAL-TIME EMIT - now io is defined
        if (io) {
          ...
          console.log(`📡 Emitted new_booking to pandit_${pandit._id}`);
        }
      }
  ...
  // Send notification to a single pandit (for direct booking)
  static async notifySinglePandit(booking, pandit, io) {  // ← Add io parameter
    try {
      const Notification = require('../models/Notification');
      const expiryTime = new Date();
      expiryTime.setHours(expiryTime.getHours() + 24);
